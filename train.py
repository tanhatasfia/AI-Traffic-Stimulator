...
import crossView
from crossView.datasets import CustomTrafficDataset
...
class Trainer:
    def __init__(self):
        ...
        self.criterion = compute_losses(device=self.device, loss_mode='focal+dice')
        ...
        self.models["decoder"] = crossView.Decoder(
            ...
            self.opt.num_class,
            occ_map_size=self.opt.occ_map_size)

        self.models["transform_decoder"] = crossView.Decoder(
            self.models["encoder"].resnet_encoder.num_ch_enc,
            self.opt.num_class,
            type="transform_decoder",
            occ_map_size=self.opt.occ_map_size)
        ...
        self.valid = Variable(torch.Tensor(np.ones((self.opt.batch_size, *self.patch))), requires_grad=False).float().cuda()
        self.fake = Variable(torch.Tensor(np.zeros((self.opt.batch_size, *self.patch))), requires_grad=False).float().cuda()
        ...
        self.val_loader = DataLoader(val_dataset, 1, True, num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)

        if self.opt.load_weights_folder != "":
            self.load_model()

        print("Using split:", self.opt.split)
        print("There are {:d} training items and {:d} validation items".format(len(train_dataset), len(val_dataset)))
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch, self.opt.num_epochs + 1):
            ...
            loss = self.run_epoch()
            output = ("Epoch: %d | lr:%.7f | Loss: %.4f | topview Loss: %.4f | transform_topview Loss: %.4f | transform Loss: %.4f" %
    ...
    def process_batch(self, inputs, validation=False):
        ...
        #print("Topview output shape:", outputs["topview"].shape)
    ...
    def run_epoch(self):
        ...
        loss = {"loss": 0.0, "topview_loss": 0.0, "transform_loss": 0.0, "transform_topview_loss": 0.0, "loss_discr": 0.0}
    ...
    def validation(self, log):
        iou = np.zeros(self.opt.num_class)
        mAP = np.zeros(self.opt.num_class)
        for batch_idx, inputs in tqdm.tqdm(enumerate(self.val_loader)):
            ...
            iou += np.array(mean_IU(pred, true))
            mAP += np.array(mean_precision(pred, true))
        iou /= len(self.val_loader)
        mAP /= len(self.val_loader)
        output = f"Epoch: {self.epoch} | Validation: "
        for i in range(self.opt.num_class):
            output += f"[Class {i}] mIOU: {iou[i]:.4f} mAP: {mAP[i]:.4f} | "
        print(output)
        log.write(output + '\n')
        log.flush()
    ...
    def save_model(self):
        ...
        optim_path = os.path.join(save_path, "adam.pth")
